...
import { type Task, TaskStatus, TASK_STATUS_ORDER } from '@/features/tasks/types';

import { KanbanCard } from './kanban-card';
import { KanbanColumnHeader } from './kanban-column-header';

const boards: TaskStatus[] = TASK_STATUS_ORDER;
...
export const DataKanban = ({ data, onChange }: DataKanbanProps) => {
  const [tasks, setTasks] = useState<TasksState>(() => {
    const initialTasks = TASK_STATUS_ORDER.reduce((acc, status) => {
      acc[status] = [];
      return acc;
    }, {} as TasksState);
  ...
  useEffect(() => {
    const newTasks = TASK_STATUS_ORDER.reduce((acc, status) => {
      acc[status] = [];
      return acc;
    }, {} as TasksState);
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="custom-scrollbar flex max-w-full items-start gap-3 overflow-x-auto pb-2">
        {boards.map((board) => (
          <div key={board} className="mx-2 min-w-[320px] flex-1 rounded-md bg-muted p-1.5">
            ...
            <Droppable droppableId={board}>
              {(provided) => (
                <div
                  ...
                  className="hide-scrollbar h-[500px] overflow-y-auto py-1.5"
                >
