...
import { type Task, type TaskComment, type TaskHistory, TaskStatus, TaskWorkType, TASK_STATUS_LABELS, TASK_WORK_TYPE_LABELS } from '@/features/tasks/types';
...
export const TaskDetailsModal = ({ task, open, onOpenChangeAction }: TaskDetailsModalProps) => {
  ...
  const membersMap = useMemo(() => {
    const entries = (members?.documents ?? []).map((member) => [member.$id, member] as const);
    return new Map(entries);
  }, [members]);

  const projectsMap = useMemo(() => {
    const entries = (projects?.documents ?? []).map((project) => [project.$id, project] as const);
    return new Map(entries);
  }, [projects]);
  ...
  const formatHistoryValue = (field: string, value?: string | null) => {
    ...
    if (field === 'status') {
      ...
      return TASK_STATUS_LABELS[status] ?? value;
    }

    if (field === 'workType') {
      const workType = value as TaskWorkType;
      return TASK_WORK_TYPE_LABELS[workType] ?? value;
    }
  ...
  const renderHistoryValue = (field: string, value?: string | null) => {
    ...
    return (
      <span className="flex items-center gap-x-2">
        {member ? <MemberAvatar name={member.name} className="size-5" fallbackClassName="text-[10px]" /> : null}
  ...
  const renderAttachmentPreview = (url: string, sizeClassName = 'h-36', imageList: string[] = []) => {
    if (isImage(url)) {
      ...
      return (
        <button
          ...
          onClick={() => setLightboxState({ images: imageList, index: Math.max(index, 0) })}
  ...
  const activityItems = useMemo(() => {
    ...
    return [...commentItems, ...historyItems].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    >
      <div className="flex flex-col gap-6 p-6 lg:flex-row">
        <div className="flex-1 space-y-6">
          ...
          <div className="space-y-2">
            <p className="text-sm font-semibold text-neutral-900">Description</p>
            {task.description ? (
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-neutral-900">Attachments</p>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    {[...imageAttachments, ...videoAttachments].map((url) => (
                      <div key={url} className="overflow-hidden rounded-md border border-neutral-200">
                        {renderAttachmentPreview(url, 'h-36', imageAttachments)}
                      </div>
                    ))}
                  </div>
                )}
                  ...
                  <div className="space-y-2">
                    {otherAttachments.map((url) => (
                      <div key={url} className="rounded-md border border-neutral-200 p-2">
                        {renderAttachmentPreview(url, 'h-28', imageAttachments)}
                      </div>
                    ))}
                  </div>
                )}
            ...
            <Tabs defaultValue="all" className="w-full">
              ...
              <TabsContent value="all" className="mt-3">
                <div className="hide-scrollbar max-h-70 space-y-4 overflow-y-auto pr-1">
                  {activityItems.length === 0 && <p className="text-sm text-neutral-500">No activity yet.</p>}
                  ...
                  {activityItems.map((item) => {
                    if (item.type === 'comment') {
                      ...
                      return (
                        <div key={comment.$id} className="flex gap-x-3">
                          <MemberAvatar name={comment.author?.name ?? 'User'} fallbackClassName="text-[10px]" />
                          <div className="flex-1 space-y-1">
                            <div className="flex items-center gap-x-2 text-xs text-neutral-500">
                              <span className="font-medium text-neutral-800">{comment.author?.name ?? 'User'}</span>
                              <TaskDate value={comment.$createdAt} className="text-xs" />
                            </div>
                            <p className="text-sm text-neutral-700">{comment.body}</p>
                            {(comment.attachments ?? []).length > 0 && (
                              <div className="grid gap-2 sm:grid-cols-2">
                                {comment.attachments?.map((url: string) => {
                                  const commentImages = (comment.attachments ?? []).filter((entry: string) => isImage(entry));
                                  return (
                                    <div key={url} className="overflow-hidden rounded-md border border-neutral-200">
                                      {renderAttachmentPreview(url, 'h-32', commentImages)}
                                    </div>
                                  );
                                })}
                              </div>
                    ...
                    const action = getHistoryAction(entry.field);

                    return (
                      <div key={entry.$id} className="flex gap-x-3">
                        <MemberAvatar name={actorName} fallbackClassName="text-[10px]" />
                        <div className="flex-1 space-y-1">
                          <div className="flex flex-col gap-y-1">
                            <p className="text-sm text-neutral-800">
                              <span className="font-medium">{actorName}</span> {action}
                            </p>
                            <TaskDate value={entry.$createdAt} className="text-xs text-neutral-500" />
                          </div>

                          {entry.field !== 'created' && (
                            <div className="flex items-center gap-x-2 text-sm text-neutral-600">
                              {renderHistoryValue(entry.field, entry.fromValue)}
                        ...
                        <div key={comment.$id} className="flex gap-x-3">
                          ...
                          <div className="flex-1 space-y-1">
                            ...
                            {(comment.attachments ?? []).length > 0 && (
                              <div className="grid gap-2 sm:grid-cols-2">
                                {comment.attachments?.map((url: string) => {
                                  const commentImages = (comment.attachments ?? []).filter((item: string) => isImage(item));
                                  return (
                                    <div key={url} className="overflow-hidden rounded-md border border-neutral-200">
                                      {renderAttachmentPreview(url, 'h-32', commentImages)}
                                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-x-2">
                        ...
                        {commentFiles.length > 0 && (
                          <span className="text-xs text-neutral-500">{commentFiles.length} files selected</span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-x-2">
                        ...
                        <Button size="sm" onClick={handleSubmitComment} disabled={isPending || isUploading}>
        ...
        <aside className="w-full shrink-0 rounded-lg mt-8 border border-neutral-200 bg-white p-4 lg:w-72">
          ...
          <div className="mt-4 space-y-4 text-sm">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-x-2">
                <MemberAvatar name={task.reporter?.name ?? 'User'} fallbackClassName="text-[10px]" />
                <span className="font-medium text-neutral-800">{task.reporter?.name ?? 'Unknown'}</span>
              </div>
            </div>

            <div className="flex items-center justify-between">
              <span className="text-neutral-500">Status</span>
              <StatusSelect taskId={task.$id} status={task.status} triggerClassName="justify-between" />
            </div>

            <div className="flex items-center justify-between">
              <span className="text-neutral-500">Work Type</span>
              <WorkTypeSelect taskId={task.$id} workType={task.workType} triggerClassName="justify-between" />
            </div>

            <div className="flex items-center justify-between">
              <span className="text-neutral-500">Project</span>
              <div className="flex items-center gap-x-2">
                <ProjectAvatar name={task.project?.name ?? 'Project'} image={task.project?.imageUrl} fallbackClassName="text-[10px]" />
                <span className="font-medium text-neutral-800">{task.project?.name ?? 'Project'}</span>
              </div>
            </div>
