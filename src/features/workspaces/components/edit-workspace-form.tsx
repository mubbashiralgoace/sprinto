...
export const EditWorkspaceForm = ({ onCancel, initialValues }: EditWorkspaceFormProps) => {
  ...
  const { data: members } = useGetMembers({ workspaceId: initialValues.$id });
  ...
  const canCopyInvite = isOwner || currentMember?.role === MemberRole.ADMIN;

  const updateWorkspaceForm = useForm<z.infer<typeof updateWorkspaceSchema>>({
    resolver: zodResolver(updateWorkspaceSchema as unknown as Parameters<typeof zodResolver>[0]),
    defaultValues: {
      ...initialValues,
  ...
  const handleCopy = () => {
    if (!canCopyInvite) {
      toast.error('You do not have permission to copy the invite link.');
      return;
    }
